 ...
 * digraphs. In scheduling problems, arranging for the completion of a set of
 * jobs, under a set of constraints, by specifying when and how the jobs are to
 * be performed, digraph models arise directly. The most important type of
 * constraints is precedence constraints, which specify that certain tasks must
 * be performed before certain others. In general, if a precedence-constrained
 * scheduling problem has a directed cycle, then there is no feasible solution.
 */
public interface DigraphOrder {
	enum Order {
		...
		TOPOLOGICAL_ORDER("Puts the vertex on a stack after visiting the next vertices");
